...
export const tokenSearchRegex =
  /(DSA_COLOR_BACKGROUND_|DSA_COLOR_TEXT_|DSA_COLOR_BORDER_|DSA_COLOR_SPECIALPAGES_|DSA_COLOR_SUBJECTS_|DSA_COLOR_CONTENTBOX_|DSA_COLOR_HIGHLIGHT_|DSA_COLOR_BUTTON_|DSA_COLOR_FEEDBACK_)\w+/g;
...
export const getThemeTokenValue = (token: string, theme: Theme): string => {
  if (typeof token === 'string' && token.includes('DSA_COLOR')) {
    const match = token.match(tokenSearchRegex);
